...
import {
  deleteAllFromMap,
  deleteMultipleFromMap,
  drawOnMap,
} from "@/utils/traffic-jam-utils/map";
import useAxios from "@/utils/traffic-jam-utils/useAxios";
import {
  getOptionsFromStreet,
  ignoreDiacriticsFilter,
} from "@/utils/traffic-jam-utils/util";
...
const StreetsDrawer = ({
  ...
  const { coordinates, setNewRoute, setNewCoordinates } =
    useContext(routeContext);

  const [options, setOptions] = useState<SelectProps["options"]>(
    getOptionsFromStreet(null, []),
  );
  ...
  useEffect(() => {
    const streets = streetsInMap.filter(
      (obj) =>
        !streetsInSelected.includes(obj.name) &&
        !streetsInRoute.includes(obj.name),
    );
  ...
  async function updateRoute(index: number) {
    ...
    for (let i = 0; i < coordinates.length - 1; i++) {
      ...
      const streetToRemove = oldStreetsInRoute?.filter(
        (item) => !newStreetsInRoute?.includes(item),
      );
