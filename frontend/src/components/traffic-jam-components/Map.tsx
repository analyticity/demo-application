...
import { drawOnMap } from "@/utils/traffic-jam-utils/map";
...
import {
  get_data_delay_alerts,
  get_route,
  reverse_geocode,
} from "@/utils/traffic-jam-utils/backendApiRequests";
import { pin } from "@/utils/traffic-jam-utils/icons";
...
type Props = {
  ...
  setMapMode: React.Dispatch<
    React.SetStateAction<"route" | "street" | "nothing">
  >;
  loading: boolean;
...
const Map = ({
  ...
  const { coordinates, setNewCoordinates, setNewRoute, route } =
        ...
        if (mapMode === "route") {
          ...
          if (last_two.length > 1) {
            ...
            if (response.streets_coord.length < 1) {
              ...
              setTimeout(() => {
                api["error"]({
                  ...
                  description:
                    "Select different points - ideally with multiple pass points",
                  placement: "bottomRight",
                });
              }, 1000);
              return;
            ...
            const newStreetsInRoute2 = [
              ...new Set([
                ...streetsInRoute,
                ...response.streets_coord.map((street) => street.street_name),
              ]),
            ];
            ...
            setNewCoordinates((prevData) => {
              ...
              newData[lastIndex] = {
                ...newData[lastIndex],
                street: response.dst_street,
              };
              newData[secondLastIndex] = {
                ...newData[secondLastIndex],
                street: response.src_street,
              };
              if (
                newData[secondLastIndex].street == newData[lastIndex].street
              ) {
                newData.pop();
              }
              return newData;
            ...
            const data = await get_data_delay_alerts(
              filter,
              new_route,
              newStreetsInRoute2,
            );
          ...
          const fetchReverseStreet = async () => {
            ...
            setNewStreetsInSelected((prevState) => [
              ...new Set([...prevState, ...streets]),
            ]);
                ...
                alert?.visible && (
                  ...
                  >
                    <Popup>
                      ...
                      <p>
                        {t("pubMillis")}:{" "}
                        {dayjs(alert["pubmillis"]).format("DD.MM.YYYY HH:mm")}
                      </p>
                    </Popup>
                  </Marker>
                ),
