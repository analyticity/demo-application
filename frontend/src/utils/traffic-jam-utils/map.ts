...
export function deleteFromMap(streetsInMap: StreetInMap[], name: string) {
  const streetsInMapToDelete = streetsInMap.filter(
 ...
 */
export function deleteMultipleFromMap(
  streetsInMap: StreetInMap[],
  streets: string[],
) {
  const streetsInMapToDelete = streetsInMap?.filter((street) =>
    streets.includes(street.name),
  );
  streetsInMapToDelete?.forEach((street) => {
    street?.lines?.forEach((line) => line.remove());
  });
  const streetsInMapStaying = streetsInMap?.filter(
    (street) => !streets.includes(street.name),
  );
