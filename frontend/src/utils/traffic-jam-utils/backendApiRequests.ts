/*
	...
	file: backendApiRequests.ts
*/

import backendApi from "./api";
import { Filter } from "./contexts";
import { queryFindStreet, queryStreetCoord } from "./queryBuilder";
...
function getRequestBodyWithRoute(
  filter: Filter,
  route,
...
export async function get_data_delay_alerts(
  filter: Filter,
  route,
  streetsInRoute: string[],
) {
...
export async function get_data_critical_streets(
  filter: Filter,
  route,
  streetsInRoute: string[],
) {
...
export async function get_data_alert_types(
  filter: Filter,
  route,
  streetsInRoute: string[],
) {
...
export async function get_streets_coord(filter: Filter, newlySelected: string) {
  const response = await backendApi.get(
    `/street_coord/?${queryStreetCoord(filter, newlySelected)}`,
  );
  return response.data;
}

export async function get_all_street_delays(
  filter: Filter,
...
export async function reverse_geocode(filter: Filter, e: L.LeafletMouseEvent) {
  const response = await backendApi.get(
    `reverse_geocode/street/?${queryFindStreet(e, filter)}`,
  );
  return response.data;
}
