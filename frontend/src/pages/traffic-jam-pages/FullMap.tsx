...
import DateSlider from "@/components/traffic-jam-components/DateSlider";
import InfoModal from "@/components/traffic-jam-components/ModalComponents/InfoModal";
import ApplicationTour from "@/components/traffic-jam-components/SidebarComponents/ApplicationTour";
import Sidebar from "@/components/traffic-jam-components/SidebarComponents/Sidebar";
import {
  get_all_street_alerts,
  get_all_street_delays,
  get_streets_coord,
} from "@/utils/traffic-jam-utils/backendApiRequests";
...
import Map from "@/components/traffic-jam-components/Map";
import { useUrlSearchParams } from "@/hooks/use-url-search-params";
import { StreetInMap } from "@/types/street-in-map";
import {
  filterContext,
  routeContext,
  streetContext,
...
import {
  ...
  RouteIcon,
  WarningSmallIcon,
} from "@/utils/traffic-jam-utils/icons";
import { deleteAllFromMap, drawOnMap } from "@/utils/traffic-jam-utils/map";
...
const FullMap = () => {
  ...
  const [buttonStyleDelay, setButtonStyleDelay] = useState<
    "default" | "primary"
  >("default");
  const [buttonStyleDelayDisabled, setButtonStyleDelayDisabled] =
    useState<boolean>(false);
  const [buttonStyleAlerts, setButtonStyleAlerts] = useState<
    "default" | "primary"
  >("default");
  const [buttonStyleAlertsDisabled, setButtonStyleAlertsDisabled] =
    useState<boolean>(false);
  const [mapMode, setMapMode] = useState<"route" | "street" | "nothing">(
    "street",
  );
  ...
  const allDelayData = async () => {
    ...
    data?.forEach((element) => {
      streets.push(
        drawOnMap(map, element?.street, element?.path, element?.color),
      );
    });
  ...
  const allAlertData = async () => {
    ...
    try {
      data = await get_all_street_alerts(
        filter,
        route,
        streetsInRoute,
        controllerAlerts.current,
      );
    } catch (error) {
      data = [];
      setTimeout(() => {
    ...
    const newData: AdressPoint = data.map((obj) => ({
      ...obj,
      visible: true,
      icon: getPinIcon(obj.type),
    }));
  ...
  useEffect(() => {
    if (mapRef.current) {
      ...
      streetsInMap?.forEach((obj) =>
        obj.lines?.forEach((line) => line?.addTo(mapRef?.current)),
      );
    }
  }, [mapRef.current]);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const isMatched = streetsInMap.some(
        (street) => street.name === newlySelected,
      );
      ...
      if (!isMatched) {
        ...
        data?.streets?.forEach((element) => {
          const newDrawedStreet: StreetInMap = drawOnMap(
            map,
            element.street_name,
            element.path,
            element.color,
  ...
  useEffect(() => {
    ...
    routeStreets?.forEach((element) => {
      const streetInMapNew: StreetInMap = drawOnMap(
        map,
        element?.street_name,
        element?.path,
        element?.color,
    ...
    setNewStreetsInMap((prevState: StreetInMap[]) => {
      ...
      newStreetsInMap.forEach((streetInMap) => {
        const index = prevState.findIndex(
          ({ name }) => name === streetInMap.name,
        );
        if (index > -1) {
          stateCopy[index] = {
            ...stateCopy[index],
            lines: [...stateCopy[index].lines, ...streetInMap.lines],
          };
        } else {
  ...
  useEffect(() => {
    if (buttonStyleAlerts == "primary") {
      allAlertData();
      const streetToRemove = streetsInSelected?.filter(
        (item) => !filter?.streets.includes(item),
      );
      const newAlertPoints = alertsPoints?.filter(
        (item) => !streetToRemove.includes(item.street),
      );
  ...
  return (
    <div>
      <Row style={{ flexFlow: "unset" }}>
        ...
        <Col
          span={23}
          style={{ position: "relative", flex: "1", maxWidth: "unset" }}
        >
