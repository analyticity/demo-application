...
import BarChartComponent from "@/components/traffic-jam-components/GraphComponents/BarChartComponent";
import ChartTimelineComponent from "@/components/traffic-jam-components/GraphComponents/ChartTimelineComponent";
import LineChartComponentV2 from "@/components/traffic-jam-components/GraphComponents/LineChartComponentV2";
import MultipleYChartComponent from "@/components/traffic-jam-components/GraphComponents/MultipleYChartComponent";
import FilterDrawer from "@/components/traffic-jam-components/SidebarComponents/FilterDrawer";
import StatsTilesComplet from "@/components/traffic-jam-components/StatsTilesComponents/StatsTileComplet";
import { useUrlSearchParams } from "@/hooks/use-url-search-params";
import {
  get_data_alert_types,
  get_data_critical_streets,
...
const Dashboard = () => {
  ...
  const [criticalStreetsAlerts, setCriticalStreetsAlerts] =
    useState<BarChartData>({ streets: [], values: [] });
  const [criticalStreetsJams, setCriticalStreetsJams] = useState<BarChartData>({
    streets: [],
    values: [],
  });
  ...
  const [loadingCriticalStreets, setLoadingCriticalStreets] =
  ...
  useEffect(() => {
    ...
    const get_critical_streets_data = async () => {
      const data = await get_data_critical_streets(
        filter,
        route,
        streetsInRoute,
      );
      setCriticalStreetsJams({
        streets: data.streets_jams,
        values: data.values_jams,
      });
      setCriticalStreetsAlerts({
        streets: data.streets_alerts,
        values: data.values_alerts,
      });
        ...
        <Col xxl={5} xl={9} lg={9} md={24} className="alerts-type-col">
          <Card title={t("tile.AlertsType")}>
            <Spin
              tip={t("loading.data")}
              size="large"
              spinning={loadingAlertTypes}
            >
              <BarChartComponent
                streets={alertTypes["basic_types_labels"]?.map((value) =>
                  t(value),
                )}
              ...
              />
              <BarChartComponent
                streets={alertTypes[selected]?.subtype_labels.map((value) =>
                  t(value),
                )}
        ...
        <Col xxl={7} xl={15} lg={15} md={24} className="graph-tiles-col">
          <Card title={t("graph.tiles.title")}>
            <Spin
              tip={t("loading.data")}
              size="large"
              spinning={loadingCriticalStreets}
            >
              <BarChartComponent
